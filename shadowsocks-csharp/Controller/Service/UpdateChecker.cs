...
namespace Photon.Controller
{
    ...
    {
        private const string UpdateURL = "https://api.github.com/repos/Evilrabbit520/Photon/releases";   //更新用的链接采用开源Github API
        ...
        public const string Version = "1.0.0.6";    //固件版本号
        ...
        private WebClient CreateWebClient()
        {
            ...
            //http.Proxy = new WebProxy(IPAddress.Loopback.ToString(), config.localPort);   //启动代理后才可以检查更新
            return http;
        }
        ...
        public class Asset : IComparable<Asset>
        {
            ...
            public static Asset ParseAsset(JObject assertJObject)
            {
                ...
                Match match = Regex.Match(name, @"^Photon-(?<version>\d+(?:\.\d+)*)(?:|-(?<suffix>.+))\.\w+$",
